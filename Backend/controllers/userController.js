...
// Delete user account and all their customers
const deleteAccount = async (req, res) => {
  try {
    // Get user id from request
    const userId = req.user._id;

    // Find all customers associated with the user
    const customers = await Customer.find({ user: userId });

    // Delete each customer image from Cloudinary
    for (let customer of customers) {
      if (customer.passport_photo.public_id) {
        try {
          // Extract the public ID of the image from the customer
          let publicId = customer.passport_photo.public_id;
    ...
    // Delete all customers associated with the user from the database
    ...
    res.status(200).json({
      status: 'success',
      message: 'Account and all associated customers and images have been deleted',
    });
  } catch (error) {
    console.log(error)
    res.status(500).json({
